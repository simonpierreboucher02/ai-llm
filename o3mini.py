#!/usr/bin/env python3
"""
OpenAI O-Series Chat Agent - Advanced Production-Ready Chat Interface

This module provides a complete chat agent implementation for OpenAI's o3-mini model
using the standard chat completions API endpoint. Features include:

- Support exclusively for o3-mini model with reasoning capabilities
...
@dataclass
class AgentConfig:
    """Configuration settings for the OpenAI O-Series Chat Agent"""
    model: str = "o3-mini"
...
class OpenAIOSeriesChatAgent:
    """Advanced OpenAI O-Series Chat Agent with persistence and streaming support"""

    # Specific configuration for o3-mini model
    MODEL_INFO = {
        "o3-mini": {
            "name": "O3 Mini",
    ...
    SUPPORTED_EXTENSIONS = {
        # ... (les extensions restent inchangées, conserver le même contenu)
    ...
    def __init__(self, agent_id: str):
        ...
        self.logger.info(f"Initialized OpenAI O-Series Chat Agent: {agent_id} with model: {self.config.model}")
    ...
    def _get_api_key(self) -> str:
        ...
        if secrets_file.exists():
            try:
                with open(secrets_file, 'r') as f:
                    ...
                    # Use o3-mini specific key or default
                    api_key = keys.get("o3-mini") or keys.get('default')
                    if api_key:
        ...
        model_display = self.MODEL_INFO.get(self.config.model, {}).get('name', self.config.model)
    ...
    def _build_api_payload(self, new_message: str, override_config: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """Build the API request payload matching the o3-mini structure"""
        # Process file inclusions
        processed_message = self._process_file_inclusions(new_message)

        # Build messages in the exact o3-mini CURL structure
        messages = []
        ...
        # Build payload matching the exact o3-mini CURL structure
        payload = {
            "model": config["model"],
            "messages": messages,
    ...
    def _get_timeout_for_reasoning(self, model: str = None, reasoning_effort: str = "medium") -> int:
        ...
        if not model_info:
            # Fallback to default o3-mini timeout
            model_info = self.MODEL_INFO["o3-mini"]
    ...
    def _make_api_request(self, payload: Dict[str, Any]) -> requests.Response:
        ...
        model_display = self.MODEL_INFO.get(model, {}).get('name', model)
            ...
            for line in response.iter_lines(decode_unicode=True):
                ...
                try:
                    ...
                    if line.startswith("data: "):
                        ...
                        # Handle o3-mini streaming format matching the example
    ...
    def call_api(self, new_message: str, override_config: Optional[Dict[str, Any]] = None) -> Generator[str, None, None]:
        """Call OpenAI o3-mini API with the new message"""
        try:
            ...
            if config_file.exists():
                try:
                    with open(config_file) as f:
                        ...
                        agent_info["model"] = config.get("model", "o3-mini")
...
def show_agent_info(agent_id: str):
    ...
    if config_file.exists():
        try:
            ...
            model_display = OpenAIOSeriesChatAgent.MODEL_INFO.get(model, {}).get('name', model)
...
def create_agent_config_interactive() -> AgentConfig:
    ...
    # o3-mini is the only model, no need for selection

    selected_model_info = OpenAIOSeriesChatAgent.MODEL_INFO.get(config.model, {})
...
def interactive_chat(agent: OpenAIOSeriesChatAgent):
    ...
    model_display = agent.MODEL_INFO.get(agent.config.model, {}).get('name', agent.config.model)
        ...
        try:
            ...
            if user_input.startswith('/'):
                ...
                if command == 'help':
                    ...
                    print(f"{Fore.YELLOW}🧠 o3-mini Model & Reasoning Timeouts:")
                    for model_key, model_info in [("o3-mini", OpenAIOSeriesChatAgent.MODEL_INFO["o3-mini"])]:
                        timeouts = model_info["reasoning_timeout"]
                        print(f"{Fore.WHITE}  {model_info['name']} ({model_key}):")
                    ...
                    for key, value in config_dict.items():
                        if key not in ['created_at', 'updated_at']:
                            if key == 'model':
                                model_name = agent.MODEL_INFO.get(str(value), {}).get('name', value)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="OpenAI O-Series Chat Agent - Advanced Reasoning AI Chat Interface",
...
Examples:
  ...
  %(prog)s --agent-id my-agent --config          # Configure agent interactively
    ...
    # Supprimer l'option --model car seul o3-mini est supporté
    # parser.add_argument("--model", choices=list(OpenAIOSeriesChatAgent.SUPPORTED_MODELS.keys()),
    #                    help="Override model (o1, o3, o3-mini, o4-mini)")
