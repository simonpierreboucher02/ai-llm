#!/usr/bin/env python3
"""
OpenAI GPT-4.1 Mini Chat Agent - Advanced Production-Ready Chat Interface

This module provides a complete chat agent implementation for OpenAI's GPT-4.1-mini model  
...
Example Usage:
    ...
    python openai_gpt41_mini_agent.py --agent-id my-agent

    # List all agents
    python openai_gpt41_mini_agent.py --list

    # Export conversation
    python openai_gpt41_mini_agent.py --agent-id my-agent --export html

    # Configure agent settings
    python openai_gpt41_mini_agent.py --agent-id my-agent --config
"""
...
@dataclass
class AgentConfig:
    """Configuration settings for the OpenAI GPT-4.1-mini Chat Agent"""
...
class OpenAIGPT41MiniChatAgent:
    """Advanced OpenAI GPT-4.1-mini Chat Agent with persistence and streaming support"""

    # Supported GPT-4.1 model variant
    SUPPORTED_MODEL = {
        "gpt-4.1-mini": {
            "name": "GPT-4.1 Mini",
    ...
    def __init__(self, agent_id: str):
        ...
        self.logger.info(f"Initialized OpenAI GPT-4.1-mini Chat Agent: {agent_id} with model: {self.config.model}")
    ...
    def _setup_logging(self):
        ...
        # Create logger
        self.logger = logging.getLogger(f"OpenAIGPT41MiniAgent_{self.agent_id}")
    ...
    def _build_api_payload(self, new_message: str, override_config: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        # Build messages in the exact GPT-4.1-mini CURL structure
        messages = []
        ...
        # Build payload matching the exact GPT-4.1-mini CURL structure
        payload = {
            "model": config["model"],
            "messages": messages,
    ...
    def _get_timeout_for_model(self, model: str = None) -> int:
        """Get appropriate timeout based on model variant"""
        if model is None:
            model = self.config.model

        model_info = self.SUPPORTED_MODEL.get(model)
        if not model_info:
            # Fallback to default gpt-4.1-mini timeout
            model_info = self.SUPPORTED_MODEL["gpt-4.1-mini"]
    ...
    def _make_api_request(self, payload: Dict[str, Any]) -> requests.Response:
        ...
        self.logger.info(f"Using timeout of {timeout}s for {model_display}")
            ...
            for line in response.iter_lines(decode_unicode=True):
                ...
                try:
                    ...
                    if line.startswith("data: "):
                        ...
                        # Handle GPT-4.1-mini streaming format
    ...
    def call_api(self, new_message: str, override_config: Optional[Dict[str, Any]] = None) -> Generator[str, None, None]:
        """Call OpenAI GPT-4.1-mini API with the new message"""
        try:
            ...
            print(f"{Fore.YELLOW}🤖 Using {model_display} (timeout: {timeout//60}min {timeout%60}s)...{Style.RESET_ALL}")
            ...
            if config_file.exists():
                try:
                    with open(config_file) as f:
                        ...
                        agent_info["model"] = config.get("model", "gpt-4.1-mini")
...
def show_agent_info(agent_id: str):
    ...
    if config_file.exists():
        try:
            ...
            model_display = OpenAIGPT41MiniChatAgent.SUPPORTED_MODEL.get(model, {}).get('name', model)

            print(f"\n{Fore.GREEN}Configuration:")
            print(f"{Fore.WHITE}  Model: {model} ({model_display})")
            print(f"  Temperature: {config.get('temperature', 1.0)}")
            print(f"  Max Tokens: {config.get('max_completion_tokens', 32768)}")
...
def create_agent_config_interactive() -> AgentConfig:
    ...
    selected_model_info = OpenAIGPT41MiniChatAgent.SUPPORTED_MODEL.get(config.model, {})
    ...
    tokens_input = input(f"Max completion tokens [{config.max_completion_tokens}]: ").strip()
    if tokens_input:
        try:
            config.max_completion_tokens = int(tokens_input)
        except ValueError:
...
def interactive_chat(agent: OpenAIGPT41MiniChatAgent):
        ...
        try:
            ...
            if user_input.startswith('/'):
                ...
                if command == 'help':
                    ...
                    print(f"{Fore.YELLOW}⚡ Model Variants & Timeouts:")
                    for model_key, model_info in agent.SUPPORTED_MODEL.items():
                        timeout = model_info["timeout"]
                        print(f"{Fore.WHITE}  {model_info['name']} ({model_key}): {timeout}s")
                    print()
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="OpenAI GPT-4.1-mini Chat Agent - Advanced AI Chat Interface",
...
Examples:
  ...
  %(prog)s --agent-id my-agent --config          # Configure agent interactively
    ...
    if args.list:
        ...
        print(f"{Fore.WHITE}{'ID':<20} {'Model':<20} {'Messages':<10} {'Last Updated':<20}")
        print("-" * 75)
    ...
    try:
        # Initialize agent
        agent = OpenAIGPT41MiniChatAgent(args.agent_id)
