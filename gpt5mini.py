#!/usr/bin/env python3
"""
OpenAI GPT-5 Mini Chat Agent - Advanced Production-Ready Chat Interface

This module provides a complete chat agent implementation for OpenAI's GPT-5-mini model  
using the standard chat completions API endpoint. Features include:

- Support for GPT-5-mini model only
...
Example Usage:
    ...
    python openai_gpt5_agent.py --agent-id my-agent

    # List all agents
    python openai_gpt5_agent.py --list

    # Export conversation
    python openai_gpt5_agent.py --agent-id my-agent --export html

    # Configure agent settings
    python openai_gpt5_agent.py --agent-id my-agent --config
"""
...
@dataclass
class AgentConfig:
    """Configuration settings for the OpenAI GPT-5 Mini Chat Agent"""
...
class OpenAIGPT5MiniChatAgent:
    """Advanced OpenAI GPT-5 Mini Chat Agent with persistence and streaming support"""

    # Supported GPT-5-mini model
    SUPPORTED_MODEL = {
        "gpt-5-mini": {
            "name": "GPT-5 Mini",
    ...
    def __init__(self, agent_id: str):
        ...
        self.logger.info(f"Initialized OpenAI GPT-5 Mini Chat Agent: {agent_id} with model: {self.config.model}")
    ...
    def _setup_logging(self):
        ...
        # Create logger
        self.logger = logging.getLogger(f"OpenAIGPT5MiniAgent_{self.agent_id}")
        ...
        if secrets_file.exists():
            try:
                with open(secrets_file, 'r') as f:
                    ...
                    # Since only one model, get the specific key or default
    ...
    def _build_api_payload(self, new_message: str, override_config: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """Build the API request payload faithfully mirroring the CURL structure"""
        # Process file inclusions
        processed_message = self._process_file_inclusions(new_message)

        # Build messages in the exact CURL structure
        messages = []
    ...
    def _get_timeout_for_reasoning(self, model: str = None, reasoning_effort: str = "medium") -> int:
        ...
        model_info = self.SUPPORTED_MODEL.get(model)
        if not model_info:
            # Fallback to default GPT-5-min timeouts
            model_info = self.SUPPORTED_MODEL["gpt-5-mini"]
    ...
    def _make_api_request(self, payload: Dict[str, Any]) -> requests.Response:
        ...
        model_display = self.SUPPORTED_MODEL.get(model, {}).get('name', model)
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt == max_retries - 1:
                    raise Exception(f"Request timed out after {timeout}s. Try reducing reasoning effort or check your network connection.")
                delay = base_delay * (2 ** attempt)
            ...
            for line in response.iter_lines(decode_unicode=True):
                ...
                try:
                    ...
                    if line.startswith("data: "):
                        ...
                        # Handle GPT-5-mini streaming format matching the example
    ...
    def call_api(self, new_message: str, override_config: Optional[Dict[str, Any]] = None) -> Generator[str, None, None]:
        """Call OpenAI GPT-5-mini API with the new message"""
        try:
            ...
            if config_file.exists():
                try:
                    with open(config_file) as f:
                        ...
                        agent_info["model"] = config.get("model", "gpt-5-mini")
...
def show_agent_info(agent_id: str):
    ...
    if config_file.exists():
        try:
            ...
            model_display = OpenAIGPT5MiniChatAgent.SUPPORTED_MODEL.get(model, {}).get('name', model)
...
def create_agent_config_interactive() -> AgentConfig:
    ...
    # Since only one model, inform the user
    selected_model_info = OpenAIGPT5MiniChatAgent.SUPPORTED_MODEL.get(config.model, {})
    print(f"{Fore.GREEN}Using GPT-5 Mini model. Details:")
    print(f"  {selected_model_info.get('description')}")
    timeouts = selected_model_info.get("reasoning_timeout", {})
    print(f"  Timeouts: Low={timeouts.get('low', '')}s, Medium={timeouts.get('medium', '')}s, High={timeouts.get('high', '')}s\n")
    ...
    if effort_input and effort_input in ["low", "medium", "high"]:
        ...
        print(f"{Fore.YELLOW}  → Timeout for {config.model} with {config.reasoning_effort} effort: {timeout}s ({timeout//60}min {timeout%60}s){Style.RESET_ALL}")
...
def interactive_chat(agent: OpenAIGPT5MiniChatAgent):
    """Interactive chat session"""
    model_display = agent.SUPPORTED_MODEL.get(agent.config.model, {}).get('name', agent.config.model)
        ...
        try:
            ...
            if user_input.startswith('/'):
                ...
                if command == 'help':
                    ...
                    print(f"{Fore.YELLOW}⚡ Model Details:")
                    print(f"{Fore.WHITE}  GPT-5 Mini ({agent.config.model}):")
                    print(f"    {agent.SUPPORTED_MODEL[agent.config.model]['description']}")
                    timeouts = agent.SUPPORTED_MODEL[agent.config.model]["reasoning_timeout"]
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="OpenAI GPT-5 Mini Chat Agent - Advanced AI Chat Interface",
...
Examples:
  ...
  %(prog)s --agent-id my-agent --config          # Configure agent interactively
    ...
    if args.list:
        ...
        print(f"{Fore.WHITE}{'ID':<20} {'Model':<15} {'Messages':<10} {'Last Updated':<20}")
        print("-" * 75)
        ...
        for agent in agents:
            ...
            model_display = OpenAIGPT5MiniChatAgent.SUPPORTED_MODEL.get(model, {}).get('name', model)
            print(f"{agent['id']:<20} {model_display:<15} {agent.get('message_count', 0):<10} {updated:<20}")
    ...
    try:
        # Initialize agent
        agent = OpenAIGPT5MiniChatAgent(args.agent_id)
