#!/usr/bin/env python3
"""
OpenAI O4-Mini Chat Agent - Production-Ready Chat Interface

This module provides a complete chat agent implementation for OpenAI's O4-mini model
using the standard chat completions API endpoint. Features include:

- Support for O4-mini model with reasoning capabilities
...
Example Usage:
    ...
    python openai_o4_mini_agent.py --agent-id my-agent

    # List all agents
    python openai_o4_mini_agent.py --list

    # Export conversation
    python openai_o4_mini_agent.py --agent-id my-agent --export html

    # Configure agent settings
    python openai_o4_mini_agent.py --agent-id my-agent --config
"""
...
@dataclass
class AgentConfig:
    """Configuration settings for the OpenAI O4-Mini Chat Agent"""
    model: str = "o4-mini"
...
class OpenAIO4MiniChatAgent:
    """Advanced OpenAI O4-Mini Chat Agent with persistence and streaming support"""
    ...
    def __init__(self, agent_id: str):
        ...
        self.logger.info(f"Initialized OpenAI O4-Mini Chat Agent: {agent_id} with model: {self.config.model}")
    ...
    def _setup_logging(self):
        ...
        # Create logger
        self.logger = logging.getLogger(f"OpenAIO4MiniAgent_{self.agent_id}")
        ...
        if secrets_file.exists():
            try:
                with open(secrets_file, 'r') as f:
                    ...
                    # Use model-specific key first, then default
    ...
    def _build_api_payload(self, new_message: str, override_config: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """Build the API request payload for O4-mini model"""
        # Process file inclusions
        processed_message = self._process_file_inclusions(new_message)

        # Build messages in the exact API structure
        messages = []
    ...
    def _get_timeout_for_reasoning(self, model: str = None, reasoning_effort: str = "medium") -> int:
        ...
        if not model_info:
            # Fallback to default o4-mini timeouts
            model_info = self.SUPPORTED_MODEL["o4-mini"]
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt == max_retries - 1:
                    raise Exception(f"Request timed out after {timeout}s. Try reducing reasoning effort or switching to a lighter model (mini).")
                delay = base_delay * (2 ** attempt)
    ...
    def call_api(self, new_message: str, override_config: Optional[Dict[str, Any]] = None) -> Generator[str, None, None]:
        """Call OpenAI O4-mini API with the new message"""
        try:
            ...
            if config_file.exists():
                try:
                    with open(config_file) as f:
                        ...
                        agent_info["model"] = config.get("model", "o4-mini")
...
def show_agent_info(agent_id: str):
    ...
    if config_file.exists():
        try:
            ...
            model_display = OpenAIO4MiniChatAgent.SUPPORTED_MODEL.get(model, {}).get('name', model)
...
def create_agent_config_interactive() -> AgentConfig:
    ...
    # Since only o4-mini is supported, no selection needed
    selected_model_info = OpenAIO4MiniChatAgent.SUPPORTED_MODEL.get(config.model, {})
    print(f"{Fore.GREEN}Selected Model: {selected_model_info.get('name', config.model)}{Style.RESET_ALL}\n")
...
def interactive_chat(agent: OpenAIO4MiniChatAgent):
        ...
        try:
            ...
            if user_input.startswith('/'):
                ...
                if command == 'help':
                    ...
                    print(f"{Fore.YELLOW}🧠 O4-Mini Model & Reasoning Timeouts:")
                    model_info = agent.SUPPORTED_MODEL.get(agent.config.model, {})
                    timeouts = model_info.get("reasoning_timeout", {})
                    print(f"{Fore.WHITE}  {model_info.get('name', agent.config.model)} ({agent.config.model}):")
                    print(f"    Low: {timeouts.get('low', 'N/A')}s, Medium: {timeouts.get('medium', 'N/A')}s, High: {timeouts.get('high', 'N/A')}s")
                    print()
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="OpenAI O4-Mini Chat Agent - Advanced Reasoning AI Chat Interface",
...
Examples:
  ...
  %(prog)s --agent-id my-agent --config          # Configure agent interactively
    ...
    try:
        # Initialize agent
        agent = OpenAIO4MiniChatAgent(args.agent_id)
