#!/usr/bin/env python3
"""
OpenAI GPT-5 Nano Chat Agent - Advanced Production-Ready Chat Interface

This module provides a complete chat agent implementation for OpenAI's GPT-5 Nano model
using the standard chat completions API endpoint. Features include:

- Support for GPT-5-nano model only
...
@dataclass
class AgentConfig:
    """Configuration settings for the OpenAI GPT-5 Nano Chat Agent"""
...
class OpenAIGPT5NanoChatAgent:
    """Advanced OpenAI GPT-5 Nano Chat Agent with persistence and streaming support"""

    # Supported GPT-5 model variants
    SUPPORTED_MODEL = {
        "gpt-5-nano": {
            "name": "GPT-5 Nano",
    ...
    def __init__(self, agent_id: str):
        ...
        self.logger.info(f"Initialized OpenAI GPT-5 Nano Chat Agent: {agent_id} with model: {self.config.model}")
    ...
    def _setup_logging(self):
        ...
        # Create logger
        self.logger = logging.getLogger(f"OpenAIGPT5NanoAgent_{self.agent_id}")
    ...
    def _get_timeout_for_reasoning(self, model: str = None, reasoning_effort: str = "medium") -> int:
        ...
        if not model_info:
            # Fallback to default gpt-5-nano timeouts
            model_info = self.SUPPORTED_MODEL["gpt-5-nano"]
            ...
            for line in response.iter_lines(decode_unicode=True):
                ...
                try:
                    ...
                    if line.startswith("data: "):
                        ...
                        # Handle GPT-5 streaming format matching the example
    ...
    def call_api(self, new_message: str, override_config: Optional[Dict[str, Any]] = None) -> Generator[str, None, None]:
        """Call OpenAI GPT-5 Nano API with the new message"""
        try:
            ...
            if config_file.exists():
                try:
                    with open(config_file) as f:
                        ...
                        agent_info["model"] = config.get("model", "gpt-5-nano")
...
def show_agent_info(agent_id: str):
    ...
    if config_file.exists():
        try:
            ...
            model_display = OpenAIGPT5NanoChatAgent.SUPPORTED_MODEL.get(model, {}).get('name', model)
...
def create_agent_config_interactive() -> AgentConfig:
    ...
    # Since only one model is supported, no need for selection
    selected_model_info = OpenAIGPT5NanoChatAgent.SUPPORTED_MODEL.get(config.model, {})
    print(f"{Fore.GREEN}Using model: {selected_model_info.get('name', config.model)} ({config.model}){Style.RESET_ALL}\n")
...
def interactive_chat(agent: OpenAIGPT5NanoChatAgent):
        ...
        try:
            ...
            if user_input.startswith('/'):
                ...
                if command == 'help':
                    ...
                    print(f"{Fore.YELLOW}⚡ Model Variants & Reasoning Timeouts:")
                    print(f"{Fore.WHITE}  {model_display} (gpt-5-nano):")
                    print(f"    Description: {agent.SUPPORTED_MODEL['gpt-5-nano']['description']}")
                    timeouts = agent.SUPPORTED_MODEL['gpt-5-nano']['reasoning_timeout']
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="OpenAI GPT-5 Nano Chat Agent - Advanced AI Chat Interface",
...
Examples:
  ...
  %(prog)s --agent-id my-agent --config          # Configure agent interactively
    ...
    # Removed --model argument since only gpt-5-nano is supported
    # Removed --effort, --temperature as these can still be configured
    ...
    if args.list:
        ...
        for agent in agents:
            ...
            model = agent.get('model', 'gpt-5-nano')
            model_display = OpenAIGPT5NanoChatAgent.SUPPORTED_MODEL.get(model, {}).get('name', model)
    ...
    try:
        # Initialize agent
        agent = OpenAIGPT5NanoChatAgent(args.agent_id)
